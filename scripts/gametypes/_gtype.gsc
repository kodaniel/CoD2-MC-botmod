...
Callback_StartGameType()
{
	...
	// Grace period
	level.readyupTime	= cvarDef("int", "scr_botmod_readyuptime", 15, 0, 300);
	// AI limit
	if (!isDefined(level.ai_max))
		level.ai_max = 64;
	level.AIlimit		= cvarDef("int", "scr_botmod_npclimit", 64, 1, level.ai_max);
	// Round limit per map
	level.roundslimit	= cvarDef("int", "scr_botmod_roundlimit", 10, 1, 100);
	// Level difficulty
	level.difficulty	= cvarDef("float", "scr_botmod_difficulty", 10, 0.1, 50);
	level.healthMultiplier = cvarDef("float", "scr_botmod_healthmultiplier", 1, 0.1, 10);
...
updateServerHostname()
{
	...
	for (;;)
	{
		placeholders[0] = level.roundslimit; // &&1
		placeholders[1] = game["roundsplayed"]; // &&2
		placeholders[2] = "Tactical"; // gametype &&3
		placeholders[3] = "TACTICAL"; // gametype capital &&4
		...
		for (i = 1; i <= level.roundslimit; i++)
		{
			...
			if (game["roundsuccess"] == "victory")
			{
				...
				for (j = 0; j < players.size; j++)
				{
					...
					if (game["roundsplayed"] == level.roundslimit) // last round survived, so increment the boss kills
					{
...
StartRound(roundNum)
{
	...
	if (!HasPlayingPlayer()) // end map, because the roundsuccess is not set to victory by the mission
		return;

	// Start a random mission
	isBossRound = roundNum == level.roundslimit;
	...
	scripts\missions\_missions::StartRandomMission(isBossRound);
