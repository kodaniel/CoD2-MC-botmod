...
setPlayerModels()
{
	mptype\assault_default::precache();
	game["assault_model"] = mptype\assault_default::main;

	mptype\medic_default::precache();
	game["medic_model"] = mptype\medic_default::main;

	mptype\engineer_default::precache();
	game["engineer_model"] = mptype\engineer_default::main;

	mptype\admin_default::precache();
	game["admin_model"] = mptype\admin_default::main;
}
...
model()
{
	...
	{
		if(self.pers["class"] == "hunter")
			[[game["assault_model"]]]();
		else if(self.pers["class"] == "medic")
			[[game["medic_model"]]]();
		else if(self.pers["class"] == "banker")
			[[game["engineer_model"]]]();
	}
...
addTestClients()
{
	...
	for(i = 0; i < testclients; i++)
	{
		...
		wait 3;
		ent[i] thread TestClient(i, class);
	}
}
...
TestClient(i, team)
{
	...
	self.account["username"] = "mcbot" + i;
	self.account["password"] = "12345";
	self notify("menuresponse", game["menu_auth"], "login");
